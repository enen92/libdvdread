                                    ...
                                    dvd_reader_dir_cb *dir_cb )
{
  ...
  if( priv != NULL && stream_cb != NULL )
  {
    ...
    ctx->rd = DVDOpenImageFile( ctx, ppath, stream_cb, have_css );
    if(!ctx->rd)
    {
        free(ctx);
        return NULL;
...
dvd_reader_t *DVDOpenFiles( void *priv, const dvd_logger_cb *logcb,
                        const char *ppath, dvd_reader_stream_cb *stream_cb, dvd_reader_dir_cb *dir_cb)
{
    return DVDOpenCommon( priv, logcb, ppath, stream_cb, dir_cb );
}
